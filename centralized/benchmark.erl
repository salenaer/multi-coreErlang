...
initialize_server_random_channels(NumberOfChannels, NumberOfUsers, ChannelsPerUser) ->
    ...
    Users = dict:from_list(lists:map(fun (Name) ->
            ...
            {Name, User} end,
        UserNames)),
    ServerPid = server_centralized:initialize_with(Users, dict:from_list([]), Channels),
    Transform=fun(UserName, {user, UserName, UserChannels})->{user, UserName, sets:to_list(UserChannels)} end,
    TransformedUsers = dict:map(Transform, Users), 
    {ServerPid, Channels, TransformedUsers}.
...
initialize_server_structured(NumberOfChannels, NumberOfUsers, UserToChannelFunction) ->
    ...
    Channels = dict:from_list(lists:map(fun (Name) ->
        ...
        ChannelNames)),
    Users = dict:from_list(lists:map(fun (Name) ->
            User = {user, Name, sets:from_list(UserToChannelFunction(Name))},
            {Name, User} end,
        UserNames)),
    ServerPid = server_centralized:initialize_with(Users, dict:from_list([]), Channels),
    Transform=fun(UserName, {user, UserName, UserChannels})->{user, UserName, sets:to_list(UserChannels)} end,
